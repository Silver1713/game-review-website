...
function setGenre(){
    ...
    var query = url.searchParams.get('genre');
    var res = query.replace('%20','\xa0')
    var c = document.getElementById('genreTitle');
    var title = res;
...
function upLoadContent(content){
    var upload = document.getElementById('genreContent');
...
function onGenreLoad(){
    
    var url = new URL(window.location.href)
    var xhr = new XMLHttpRequest()
    setGenre();
    if (url.searchParams.get('genre') == 'Adventure'){
        xhr.abort()
        xhr.open('GET','/games/search?genre=Adventure')
    ...
    if (url.searchParams.get('genre') == 'MMORPG'){
        xhr.open('GET','/games/search?genre=MMORPG')
        xhr.onreadystatechange = function(){
            if ((this.readyState == 4) && (this.status == 200)){
                var json = JSON.parse(this.responseText);
                if (json){
