...
function setPlatform(){
    ...
    var query = url.searchParams.get('plat');
    var res = query.replace('%20','\xa0')
    var c = document.getElementById('platTitle');
    var title = res;
...
function upLoadContent(content){
    var upload = document.getElementById('contentPlat');
...
function onPlatLoaded(){
    
    var url = new URL(window.location.href)
    var xhr = new XMLHttpRequest()
    setPlatform();
    if (url.searchParams.get('plat') == 'Windows'){
        xhr.abort()
        xhr.open('GET','/games/search?platform=WIN')
    ...
    if (url.searchParams.get('plat') == 'Xbox'){
        xhr.open('GET','/games/search?platform=XBOX')
        xhr.onreadystatechange = function(){
            if ((this.readyState == 4) && (this.status == 200)){
                var json = JSON.parse(this.responseText);
                if (json){
